...
public class SavingAccountService implements ISavingAccountService{
    ...
    public SavingAccountService(IAccountDAO iAccountDAO, IOperationDAO iOperationDAO) {
    ...
    public Optional<List<Account>> filter(String query) {
        List<Account> savingAccounts = _iAccountDAO.findAll();

        List<Account> filteredAccounts = savingAccounts.stream()
                .filter(
                        savingAccount ->
                                query.equals(savingAccount.get_createdAt().toString()) ||
                                query.equals(savingAccount.get_status().toString())
                ).toList();
    ...
    public Optional<Account> findByOperation(int number) {
        ...
        return operation.flatMap(Operation::get_savingAccount);
    }
    @Override
